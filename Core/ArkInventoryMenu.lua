	...
	ArkInventory.Lib.Dewdrop:Open( frame,
		...
		"children", function( level, value )
			...
			if level == 1 then
				...
				do
					local modifier = ArkInventory.db.option.restack.modifier or "NONE"
					local modifierLabel
					if modifier == "SHIFT" then
						modifierLabel = ArkInventory.Localise["RESTACK_MODIFIER_SHIFT"]
					elseif modifier == "CTRL" then
						modifierLabel = ArkInventory.Localise["RESTACK_MODIFIER_CTRL"]
					elseif modifier == "ALT" then
						modifierLabel = ArkInventory.Localise["RESTACK_MODIFIER_ALT"]
					else
						modifierLabel = ArkInventory.Localise["RESTACK_MODIFIER_NONE"]
					end
					ArkInventory.Lib.Dewdrop:AddLine(
						"text", string.format( "%s: %s%s%s", ArkInventory.Localise["RESTACK_MODIFIER"], LIGHTYELLOW_FONT_COLOR_CODE, modifierLabel, FONT_COLOR_CODE_CLOSE ),
						"tooltipTitle", ArkInventory.Localise["RESTACK_MODIFIER"],
						"tooltipText", ArkInventory.Localise["RESTACK_MODIFIER_DESC"],
						"hasArrow", true,
						"value", "RESTACK_MODIFIER"
					)
				end
			...
			if level == 2 and value == "RESTACK_MODIFIER" then

				ArkInventory.Lib.Dewdrop:AddLine(
					"text", ArkInventory.Localise["RESTACK_MODIFIER"],
					"isTitle", true
				)

				local current = ArkInventory.db.option.restack.modifier or "NONE"

				local modifierOptions = {
					{ key = "NONE",  label = ArkInventory.Localise["RESTACK_MODIFIER_NONE"]  },
					{ key = "SHIFT", label = ArkInventory.Localise["RESTACK_MODIFIER_SHIFT"] },
					{ key = "CTRL",  label = ArkInventory.Localise["RESTACK_MODIFIER_CTRL"]  },
					{ key = "ALT",   label = ArkInventory.Localise["RESTACK_MODIFIER_ALT"]   },
				}
				...
				for _, opt in ipairs( modifierOptions ) do
					...
					ArkInventory.Lib.Dewdrop:AddLine(
						...
						"closeWhenClicked", true,
						"func", function( )
							ArkInventory.db.option.restack.modifier = k
						end
					)
				end
