			...
			if numSlots > 0 then
				if bagFamily > 0 then
					...
					if map.loc_id_storage == ArkInventory.Const.Location.AccountBank then
						-- In Midnight, account bank tabs can be assigned to hold class reagents.
						-- Route those tabs to the REAGENT bucket so restack treats them like ReagentBank.
						local isReagentTab = false
						local storage = ArkInventory.Codex.GetStorage( nil, map.loc_id_storage )
						if storage then
							local bag_data = storage.data.location[map.loc_id_storage].bag[map.bag_id_storage]
							if bag_data and bag_data.df then
								local ClassReagents = Enum.BagSlotFlags and Enum.BagSlotFlags.ClassReagents or -6
								isReagentTab = FlagsUtil.IsSet( bag_data.df, ClassReagents )
							end
						end
						if isReagentTab then
...
function ArkInventory.Restack( loc_id_window )
	if ArkInventory.Global.Location[loc_id_window].ClientCheck then
		if ArkInventory.db.option.restack.enable then
			...
			-- modifier key gate: if a modifier is required, check it is currently held
			local required = ArkInventory.db.option.restack.modifier or "NONE"
			if required ~= "NONE" then
				local held = false
				if required == "SHIFT" and IsShiftKeyDown( ) then
					held = true
				elseif required == "CTRL" and IsControlKeyDown( ) then
					held = true
				elseif required == "ALT" and IsAltKeyDown( ) then
					held = true
				end
				if not held then
					ArkInventory.OutputWarning( string.format( ArkInventory.Localise["RESTACK_MODIFIER_WARN"], required ) )
					return
				end
			end
