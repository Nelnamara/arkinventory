...
ArkInventory.Const.DatabaseDefaults.global = {
	["option"] = {
		...
		["restack"] = {
			["enable"] = true,
			["modifier"] = "NONE", -- modifier key required to trigger restack: NONE, SHIFT, CTRL, ALT
...
function ArkInventory.OnInitialize( )
	...
	ArkInventory.Const.BLIZZARD.Events = {
		...
		--["AUCTION_OWNED_LIST_UPDATE"] = "EVENT_ARKINV_AUCTION_UPDATE", -- removed in Midnight 12.x
		...
		--["PLAYERBANKBAGSLOTS_CHANGED"] = "EVENT_ARKINV_BANK_SLOT", -- removed in Midnight 12.x
		--["PLAYER_ACCOUNT_BANK_TAB_SLOTS_CHANGED"] = "EVENT_ARKINV_BANK_SLOT",

		--["REAGENTBANK_PURCHASED"] = "EVENT_ARKINV_BANK_TAB", -- removed in Midnight 12.x
		["REAGENTBANK_UPDATE"] = "EVENT_ARKINV_REAGENTBANK_UPDATE",
		--["PLAYERREAGENTBANKSLOTS_CHANGED"] = "EVENT_ARKINV_REAGENTBANK_UPDATE", -- removed in Midnight 12.x
		...
		--["OBLITERUM_FORGE_SHOW"] = "EVENT_ARKINV_OBLITERUM_ENTER", -- removed in Midnight 12.x
		--["OBLITERUM_FORGE_CLOSE"] = "EVENT_ARKINV_OBLITERUM_LEAVE", -- removed in Midnight 12.x
		...
		--["VOID_STORAGE_UPDATE"] = "EVENT_ARKINV_VOID_UPDATE", -- Void Storage removed in Shadowlands
		--["VOID_STORAGE_CONTENTS_UPDATE"] = "EVENT_ARKINV_VOID_UPDATE", -- Void Storage removed in Shadowlands
		--["VOID_STORAGE_DEPOSIT_UPDATE"] = "EVENT_ARKINV_VOID_UPDATE", -- Void Storage removed in Shadowlands
		--["VOID_TRANSFER_DONE"] = "EVENT_ARKINV_VOID_UPDATE", -- Void Storage removed in Shadowlands
		...
		--["SPELL_UPDATE_COOLDOWN"] = "EVENT_ARKINV_UPDATE_COOLDOWN",     -- fires for all players; too noisy
		--["ACTIONBAR_UPDATE_COOLDOWN"] = "EVENT_ARKINV_UPDATE_COOLDOWN", -- fires for all players; too noisy
		["BAG_UPDATE_COOLDOWN"] = "EVENT_ARKINV_UPDATE_COOLDOWN",         -- throttled to 1s via bucket; safe to enable
		--["PET_BAR_UPDATE_COOLDOWN"] = "EVENT_ARKINV_UPDATE_COOLDOWN",   -- fires for all players; too noisy
...
function ArkInventory.OnEnable( )
	...
	-- Defer to next frame: LoadAddOn("Blizzard_Collections") inside BlizzardAPIHook
	-- fires events synchronously in Midnight 12.x, and any RegisterEvent() call that
	-- happens inside that tainted call stack triggers ADDON_ACTION_FORBIDDEN.
	C_Timer.After( 0, function() ArkInventory.BlizzardAPIHook() end )
...
--				return BattlePetToolTip_Show( tooltipInfo.battlePetSpeciesID, tooltipInfo.battlePetLevel, tooltipInfo.battlePetBreedQuality, tooltipInfo.battlePetMaxHealth, tooltipInfo.battlePetPower, tooltipInfo.battlePetSpeed, tooltipInfo.battlePetName )
			...
			if IsModifiedClick("COMPAREITEMS") or ArkInventory.CrossClient.GetCVarBool( "alwaysCompareItems" ) then
				ArkInventory.ShowCompareItem( GameTooltip )
			end
...
function ArkInventory.Frame_Status_Update_Tracking( loc_id_window )
	...
	if codex.style.status.currency.show then
		...
		-- always reset scroll arrows; they will be re-shown below if needed
		frame.ArkScrollLeft:Hide( )
		frame.ArkScrollRight:Hide( )
		-- hide the vertical scrollbar injected by HybridScrollFrameTemplate
		if ArkScroll.scrollBar then ArkScroll.scrollBar:Hide( ) end

		if tw <= aw then

			-- all tokens fit on the top line alongside money/empty-text
			...
			if tw <= aw then
				...
				-- tokens fit on the expanded second line
				ArkScroll:SetHorizontalScroll( 0 )
				...
				-- tokens overflow the second line — show left/right scroll arrows
				local btnW = codex.style.status.font.height
				offset = math.floor( ( codex.style.status.font.height * 2 + ArkInventory.Const.Frame.Status.Padding * 6 ) / 2 )

				ArkScroll:SetPoint( "LEFT", frame, offset, 0 )
				ArkScroll:SetPoint( "RIGHT", frame, 0 - offset, 0 )

				-- clamp current scroll position so it doesn't exceed the new range
				local maxScroll = tw - ( ArkScroll:GetWidth( ) or 0 )
				if maxScroll < 0 then maxScroll = 0 end
				ArkScroll.range = maxScroll
				ArkScroll.stepSize = ArkInventory.Const.Frame.Scroll.stepSize
				local cur = ArkScroll:GetHorizontalScroll( )
				if cur > maxScroll then ArkScroll:SetHorizontalScroll( maxScroll ) end

				frame.ArkScrollLeft:Show( )
				frame.ArkScrollLeft:SetPoint( "RIGHT", ArkScroll, "LEFT", 0 - ArkInventory.Const.Frame.Status.Padding, 0 )
				frame.ArkScrollLeft:SetWidth( btnW )
				frame.ArkScrollLeft:SetHeight( btnW )

				frame.ArkScrollRight:Show( )
				frame.ArkScrollRight:SetPoint( "LEFT", ArkScroll, "RIGHT", ArkInventory.Const.Frame.Status.Padding, 0 )
				frame.ArkScrollRight:SetWidth( btnW )
				frame.ArkScrollRight:SetHeight( btnW )
...
function ArkInventory.HorizontalScrollBar_OnClick_Left( frame )
	...
	v = v - ( parent.stepSize or ArkInventory.Const.Frame.Scroll.stepSize )
	if v < 0 then v = 0 end
