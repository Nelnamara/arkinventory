...
function ArkInventory.ShowCompareItem( tooltip )
	-- In Midnight, GameTooltip_ShowCompareItem checks tooltip.hasCooldown
	-- and suppresses the comparison panel when it is truthy.  SetBagItem /
	-- SetInventoryItem write this field onto the tooltip frame as a side-
	-- effect for items currently on cooldown.  We want the comparison to
	-- show regardless of cooldown state (the item stats don't change while
	-- on cooldown), so we clear the field for the duration of the call and
	-- then restore it.
	local saved = tooltip.hasCooldown
	tooltip.hasCooldown = nil
	GameTooltip_ShowCompareItem( tooltip )
	tooltip.hasCooldown = saved
end
